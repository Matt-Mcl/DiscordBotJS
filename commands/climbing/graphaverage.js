...
module.exports = {  
  ...
  async execute(msg, args, climbingData) {
    ...
    const startOfData = new Date('07/09/2021');
    let daysSinceStart = Math.ceil((Math.abs(startOfData - start) / 86400000) / 7) * 7;

    if (inputDay === 'thursday') daysSinceStart -= 7;
    ...
    if (hours < 10 && difference === 0) {
      ...
      start.setDate(start.getDate() - (daysSinceStart - 7));
    }

    let dates = [];

    for (let i = 1; i < (daysSinceStart + 1)/7; i++) {
    ...
    let graphData = await createGraph(climbingData, dates, { day: args[0].charAt(0).toUpperCase() + args[0].slice(1), show: "true" });
    
    let myChart = new ChartJsImage();
    myChart.setConfig(graphData);
